...
function buildSparePieces(color) {
  var pieces = ['wP', 'wK', 'wQ', 'bP', 'bK', 'bQ'];
  if (color === 'black') {
    pieces = [ 'wN', 'wR', 'wB', 'bN', 'bR', 'bB'];
  }
...
function buildSparePiecesForPlayer(player) {
  var allPieces=[
    [ 'wN', 'wR', 'wB'],
    ['bN', 'bR', 'bB'],
    ['wP', 'wK', 'wQ'],
...
function animateSparePieceToSquare(piece, dest, completeFn) {
  ...
  var left = cfg.boardEl.offset().left + cfg.boardEl.width() * .85 / 2
  var top = cfg.boardEl.offset().top - cfg.boardEl.height() * .15
...
function drawBoard() {
  ...
  if (cfg.sparePieces === true) {
    if (cfg.sparePiecesPlayer) {
      sparePiecesBottomEl.html(buildSparePiecesForPlayer(cfg.sparePiecesPlayer));
    } else {
